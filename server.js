...
var server = http.createServer(function (request, response) {
  ...
  if (path === "/") {
    ...
    response.write(fs.readFileSync('public/message.xml'))
    response.end()
    ...
    response.statusCode = 200
    response.setHeader('Content-Type','text/json;charset=utf-8')
    response.write(fs.readFileSync('public/2.json'))
    response.end()
  }
  else if(path ==='/3.json'){
    response.statusCode = 200
    response.setHeader('Content-Type','text/json;charset=utf-8')
    response.write(fs.readFileSync('public/3.json'))
    response.end()
  }
  else {
    response.statusCode = 404;
    response.setHeader("Content-Type", "text/html;charset=utf-8");
    response.write(`你访问的页面不存在呢——your path is not defined`);
    response.end();
  }
